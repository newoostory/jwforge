...
| XL (Complex) | Architecture changes, many modules | 3+ | Team + full design |
...
For all complexities (M/L/XL): display the estimate and auto-proceed. No confirmation required.
...
| XL (Complex) | Full design -- architecture diagram included | Display and auto-proceed |
...
| XL design needs revision (per Reviewer) | Conductor sends feedback to Architect for redesign (max 2 times) |

**For L complexity:** Display architecture summary to user: "Here's the plan: ..." then proceed.
**For XL complexity:** Display full architecture and auto-proceed.
...
2. Send shutdown request to Architect: `SendMessage(to="architect", message={type: "shutdown_request"})`
3. Call TeamDelete (only Architect is in the team; Reviewer subagents have already terminated)
4. Move `.jwforge/current/*` -> `.jwforge/archive/{YYYYMMDD-HHmmss}-{task-name-slug}/`
